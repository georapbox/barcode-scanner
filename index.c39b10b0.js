...
`;class tT extends HTMLElement{constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tz.content.cloneNode(!0))}get value(){return this.getAttribute("value")}set value(t){this.setAttribute("value",t)}static get observedAttributes(){return["value"]}attributeChangedCallback(t,e,o){"value"===t&&e!==o&&this.#R(this.value)}connectedCallback(){if(this.#L("value"),!tC()){let t=this.shadowRoot.querySelector("web-share");t&&(t.hidden=!0)}}async #R(t){let e;let o=this.shadowRoot.querySelector(".result"),i=o?.querySelector(".result__actions"),r=o?.querySelector(".result__item");r&&r.remove();try{let[,o]=await Y();new URL(t),(e=document.createElement("a")).href=t,window.requestAnimationFrame(()=>e.focus()),o?.openWebPageSameTab||(e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")),o?.openWebPage&&e.click()}catch{e=document.createElement("span")}e.className="result__item",e.classList.toggle("result__item--no-barcode",t===F),e.textContent=t,o?.insertBefore(e,i);let s=t!==F,a=o?.querySelector("custom-clipboard-copy"),n=o?.querySelector("web-share");a&&s?(a.setAttribute("value",t),a.hidden=!1):(a.hidden=!0,a.removeAttribute("value")),n&&tC()&&s?(n.setAttribute("share-text",t),n.hidden=!1):(n.hidden=!0,n.removeAttribute("share-text"))}#L(t){if(Object.prototype.hasOwnProperty.call(this,t)){let e=this[t];delete this[t],this[t]=e}}static defineCustomElement(t="scan-result"){"undefined"==typeof window||window.customElements.get(t)||window.customElements.define(t,tT)}}tT.defineCustomElement(),async function(){let t;let e=document.querySelector("a-tab-group"),o=document.querySelector("video-capture"),i=document.getElementById("cameraPanel"),r=document.getElementById("filePanel"),s=document.getElementById("scanInstructions"),a=document.getElementById("scanBtn"),n=document.getElementById("dropzone"),l=document.querySelector("resize-observer"),d=document.getElementById("scanFrame"),c=document.getElementById("torchButton"),h=document.getElementById("globalActions"),u=document.getElementById("historyBtn"),m=document.getElementById("historyDialog"),p=document.getElementById("settingsBtn"),b=document.getElementById("settingsDialog"),g=document.forms["settings-form"],v=document.getElementById("cameraSelect"),f=!0;tt()&&(h?.removeAttribute("hidden"),m?.removeAttribute("hidden"),b?.removeAttribute("hidden"));let{barcodeReader:y,barcodeFormats:w,barcodeReaderError:E}=await tc.init();if(E){let t=document.getElementById("barcodeReaderError");f=!1,h?.setAttribute("hidden",""),e?.setAttribute("hidden",""),t?.removeAttribute("hidden"),t.textContent=E?.message;return}o.addEventListener("video-capture:video-play",S,{once:!0}),o.addEventListener("video-capture:error",function(t){let e=t.detail.error;if("NotFoundError"===e.name)return;let o="NotAllowedError"===e.name?"Permission to use webcam was denied or video Autoplay is disabled. Reload the page to give appropriate permissions to webcam.":e.message;i.innerHTML=`<div class="alert alert-danger" role="alert" style="margin: 0;">${o}</div>`},{once:!0}),tv.defineCustomElement();let A=o?.shadowRoot,x=A?.querySelector("video"),k=A?.querySelector('[part="actions-container"]');async function C(){s?.removeAttribute("hidden");try{let e=await y.detect(x),o=e?.rawValue??"";if(!o)throw Error(F);window.cancelAnimationFrame(t),ts(i,o),to(o),s?.setAttribute("hidden",""),a?.removeAttribute("hidden"),d?.setAttribute("hidden",""),k?.setAttribute("hidden",""),tl();return}catch{}f&&(t=window.requestAnimationFrame(()=>C()))}async function S(t){d?.removeAttribute("hidden"),td(t.detail.video,d),C();let e=t.target.getTrackSettings(),i=t.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(c?.removeAttribute("hidden"),o.hasAttribute("torch")&&tu({el:c,isTorchOn:!0})),e?.zoom&&i?.zoom){let t=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=e?.zoom||1;t?.removeAttribute("hidden"),r.textContent=n,t.addEventListener("click",t=>{let e=t.target.closest('[data-action="zoom-in"]'),i=t.target.closest('[data-action="zoom-out"]');e&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n,o.zoom=n})}let s=await tv.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,v.appendChild(o)}),s.length>1&&v?.removeAttribute("hidden")}n.accept=H.join(","),th(g),function(t){if(!Array.isArray(t)||0===t.length)return;let e=document.getElementById("supportedFormats");e&&(e.textContent=`Supported formats: ${t.join(", ")}`)}(w),te((await K())[1]||[]),a.addEventListener("click",function(){a?.setAttribute("hidden",""),d?.removeAttribute("hidden"),k?.removeAttribute("hidden"),function(t){if(!t)return;let e=t.querySelector("scan-result");e?.remove()}(i),C()}),e.addEventListener("a-tab-show",Q(function(t){let e=t.detail.tabId,o=document.querySelector("video-capture");if("cameraTab"===e){if(f=!0,!o)return;let t=null!=i.querySelector("scan-result");if(o.loading||t||(d?.removeAttribute("hidden"),k?.removeAttribute("hidden"),C()),"function"==typeof o.startVideoStream){let t=v?.value||void 0;o.startVideoStream(t)}}else"fileTab"===e&&(f=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream(),d?.setAttribute("hidden",""),k?.setAttribute("hidden",""))},250)),n.addEventListener("files-dropzone-drop",function(t){!function(t){if(!t)return;let e=new Image,o=new FileReader;o.onload=o=>{let i=o.target.result;e.onload=async()=>{try{let t=await y.detect(e),o=t?.rawValue??"";if(!o)throw Error(F);ts(r,o),to(o),tl()}catch(t){ts(r,F),tl({success:!1})}},e.src=i,e.alt="Image preview",n.replaceChildren();let s=document.createElement("div");s.className="dropzone-preview",s.setAttribute("aria-hidden","true");let a=document.createElement("div");a.className="dropzone-preview__image-wrapper";let l=document.createElement("div");l.className="dropzone-preview__file-name",l.textContent=t.name,a.appendChild(e),s.appendChild(a),s.appendChild(l),n.prepend(s)},o.readAsDataURL(t)}(t.detail.acceptedFiles[0])}),l.addEventListener("resize-observer:resize",function(){td(o.shadowRoot.querySelector("video"),d)}),p.addEventListener("click",function(){b.open=!0}),g.addEventListener("change",function(t){let e={};t.currentTarget.querySelectorAll('input[type="checkbox"]').forEach(t=>e[t.name]=t.checked),X(e)}),u.addEventListener("click",function(){m.open=!0}),m.addEventListener("click",function(t){let e=t.target;if(e.closest('[data-action="delete"]')){let t=e.closest("li").dataset.value;if(window.confirm(`Delete history item ${t}?`)){ti(t);return}}if(e.closest("#emptyHistoryBtn")&&window.confirm("Empty history? This action cannot be undone.")){tr();return}}),c.addEventListener("click",function(t){o.torch=!o.torch,tu({el:t.currentTarget,isTorchOn:o.hasAttribute("torch")})}),v.addEventListener("change",function(t){if("function"!=typeof o.restartVideoStream)return;let e=t.target.value||void 0;o.restartVideoStream(e)}),document.addEventListener("visibilitychange",function(){if("cameraTab"===e.querySelector("[selected]").getAttribute("id")){if("hidden"===document.visibilityState)f=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream();else{f=!0;let t=document.querySelector("video-capture");if(!t)return;if(t.loading||i.querySelector("scan-result")||C(),"function"==typeof t.startVideoStream){let e=v?.value||void 0;t.startVideoStream(e)}}}}),document.addEventListener("keydown",function(t){"Escape"===t.key&&function(){let t=e.querySelector("#cameraTab").hasAttribute("selected"),o=!a.hidden,i=b.hasAttribute("open"),r=m.hasAttribute("open");o&&t&&!(i||r)&&a.click()}()})}();
//# sourceMappingURL=index.c39b10b0.js.map
