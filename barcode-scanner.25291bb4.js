...
`;class tB extends HTMLElement{#B=null;#H=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tF.content.cloneNode(!0))}async connectedCallback(){this.#B=this.shadowRoot?.getElementById("historyList"),this.#H=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#P((await ts())[1]||[]),this.#B?.addEventListener("click",this.#$),this.#H?.addEventListener("click",this.#V)}disconnectedCallback(){this.#B?.removeEventListener("click",this.#$),this.#H?.removeEventListener("click",this.#V)}async add(t){if(!t)return;let e={type:"add",message:"Error adding barcode to history"},[o,i=[]]=await ts();if(o||!Array.isArray(i))return this.#j("bs-history-error",e),o;if(i.find(e=>e===t))return;let r=[...i,t],[s]=await ta(r);return s?(td("Error setting history",s),this.#j("bs-history-error",e),s):(this.#B?.insertBefore(this.#U(t),this.#B.firstElementChild),this.#j("bs-history-success",{type:"add",message:"Barcode added to history"}),null)}async remove(t){if(!t)return;let e={type:"remove",message:"Error removing barcode from history"},[o,i=[]]=await ts();if(o||!Array.isArray(i))return this.#j("bs-history-error",e),o;let r=i.filter(e=>e!==t),[s]=await ta(r);if(s)return td("Error setting history",s),this.#j("bs-history-error",e),s;let a=this.#B?.querySelector(`li[data-value="${t}"]`);return a?.remove(),this.#j("bs-history-success",{type:"remove",message:"Barcode removed from history"}),null}async empty(){let[t]=await ta([]);return t?(td("Error setting history",t),this.#j("bs-history-error",{type:"empty",message:"Error emptying history"}),t):(this.#B?.replaceChildren(),this.#j("bs-history-success",{type:"empty",message:"History emptied successfully"}),null)}#P(t){if(!this.#B)return;this.#B.replaceChildren();let e=document.createDocumentFragment();[...t].reverse().forEach(t=>e.appendChild(this.#U(t))),this.#B.appendChild(e)}#U(t){let e,o=document.createElement("li");o.setAttribute("data-value",t);try{new URL(t),(e=document.createElement("a")).href=t,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=t;let i=document.createElement("div");i.className="actions";let r=document.createElement("custom-clipboard-copy"),s=r.shadowRoot?.querySelector("button");r.setAttribute("only-icon",""),r.setAttribute("value",t),s?.setAttribute("aria-label",`Copy to clipboard ${t}`),i.appendChild(r);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("aria-label",`Remove from history ${t}`),a.innerHTML=`
...
//# sourceMappingURL=barcode-scanner.25291bb4.js.map
