...
`,tA=document.createElement("template");tA.innerHTML=`
  ...
  <slot></slot>
`;class t_ extends HTMLElement{#C={};#S=null;#T=null;constructor(){super(),this.#C=this.getSupportedConstraints(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tA.content.cloneNode(!0))}static get observedAttributes(){return["no-image","pan","tilt","zoom","torch"]}attributeChangedCallback(t,e,o){if(!this.isConnected)return;let i=this.getTrackCapabilities();if("zoom"===t&&e!==o&&"zoom"in this.#C){let t=!!("zoom"in i&&i.zoom?.min&&i.zoom?.max)&&this.zoom>=i.zoom.min&&this.zoom<=i.zoom.max;"number"==typeof this.zoom&&t&&this.#z("zoom",this.zoom)}"torch"===t&&e!==o&&"torch"in this.#C&&this.#z("torch",this.torch)}async connectedCallback(){if(this.#R("autoPlay"),this.#R("facingMode"),this.#R("zoom"),this.#R("torch"),this.#O(),this.#T?.addEventListener("loadedmetadata",this.#N),!t_.isSupported())return this.#M("initialization","not-supported",Error("MediaDevices.getUserMedia() is not supported in this browser"));this.autoPlay&&this.startVideoStream()}disconnectedCallback(){this.stopVideoStream(),this.#T?.removeEventListener("loadedmetadata",this.#N)}get autoPlay(){return this.hasAttribute("auto-play")}set autoPlay(t){this.toggleAttribute("auto-play",!!t)}get facingMode(){let t=this.getAttribute("facing-mode");return"user"!==t?"environment":t}set facingMode(t){this.setAttribute("facing-mode",t)}get zoom(){return Number(this.getAttribute("zoom"))||1}set zoom(t){this.setAttribute("zoom",null!=t?t.toString():t)}get torch(){return this.hasAttribute("torch")}set torch(t){this.toggleAttribute("torch",!!t)}get loading(){return this.hasAttribute("loading")}#O(){if(this.#T)return;let t=document.createElement("video");t.setAttribute("part","video"),t.setAttribute("playsinline",""),t.setAttribute("muted",""),t.setAttribute("disablepictureinpicture",""),this.shadowRoot?.prepend(t),this.#T=t}#M(t,e,o){this.dispatchEvent(new CustomEvent(`${tw}:error`,{bubbles:!0,composed:!0,detail:{source:t,reason:e,error:o}}))}#N=async t=>{let e=t.target;try{await e.play(),this.dispatchEvent(new CustomEvent(`${tw}:video-play`,{bubbles:!0,composed:!0,detail:{video:e}}))}catch(e){let t=e instanceof DOMException&&"NotAllowedError"===e.name?"user-gesture-required":"playback-failed";this.#M("playback",t,e)}finally{this.removeAttribute("loading")}};#z(t,e){var o,i,r;if(!this.#S)return;let[s]=this.#S.getVideoTracks(),a=this.getTrackCapabilities(),n=this.getTrackSettings(),l="pan"===t||"tilt"===t||"zoom"===t?(o=Number(e),i=a[t]?.min||1,r=a[t]?.max||1,Number.isNaN(i)&&(i=0),Number.isNaN(r)&&(r=0),Math.min(Math.max(o,Math.min(i,r)),Math.max(i,r))):e;t in n&&s.applyConstraints({advanced:[{[t]:l}]}).catch(()=>{})}#R(t){if(Object.prototype.hasOwnProperty.call(this,t)){let e=this[t];delete this[t],this[t]=e}}async startVideoStream(t){if(!t_.isSupported()||this.#S)return;this.setAttribute("loading","");let e={video:{facingMode:{ideal:this.facingMode},pan:!0,tilt:!0,zoom:!0,torch:this.torch},audio:!1};if("string"==typeof t&&t.trim().length>0&&(e.video.deviceId={exact:t}),"string"==typeof this.cameraResolution&&this.cameraResolution.trim().length>0){let[t=0,o=0]=this.cameraResolution.split("x").map(t=>Number(t));t>0&&o>0&&(e.video.width=t,e.video.height=o)}try{this.#S=await navigator.mediaDevices.getUserMedia(e),this.#T&&(this.#T.srcObject=this.#S),this.#z("pan",this.pan),this.#z("tilt",this.tilt),this.#z("zoom",this.zoom)}catch(e){let t=e instanceof DOMException&&"NotAllowedError"===e.name?"camera-access-denied":"camera-failed";this.#M("camera",t,e),this.removeAttribute("loading")}}restartVideoStream(t){this.#S&&this.#T&&this.stopVideoStream(),this.startVideoStream(t)}stopVideoStream(){if(!this.#T||!this.#S)return;let[t]=this.#S.getVideoTracks();t?.stop(),this.#T.srcObject=null,this.#S=null}async playVideo(t={}){let{emit:e=!1}=t;if(this.#T&&this.#S)try{await this.#T.play(),e&&this.dispatchEvent(new CustomEvent(`${tw}:video-play`,{bubbles:!0,composed:!0,detail:{video:this.#T}}))}catch(e){let t=e instanceof DOMException&&"NotAllowedError"===e.name?"user-gesture-required":"playback-failed";this.#M("playback",t,e)}}stopVideo(){this.#T&&this.#S&&this.#T.pause()}getSupportedConstraints(){return t_.isSupported()&&navigator.mediaDevices.getSupportedConstraints()||{}}getTrackCapabilities(){if(!this.#S)return{};let[t]=this.#S.getVideoTracks();return t&&"function"==typeof t.getCapabilities&&t.getCapabilities()||{}}getTrackSettings(){if(!this.#S)return{};let[t]=this.#S.getVideoTracks();return t&&"function"==typeof t.getSettings&&t.getSettings()||{}}static async getVideoInputDevices(){return navigator.mediaDevices&&navigator.mediaDevices.enumerateDevices?(await navigator.mediaDevices.enumerateDevices()||[]).filter(t=>"videoinput"===t.kind&&!!t.deviceId):[]}static isSupported(){return!!navigator.mediaDevices?.getUserMedia}static defineCustomElement(t=tw){"u">typeof window&&!window.customElements.get(t)&&window.customElements.define(t,t_)}}var tx="clipboard-copy",tL="success",tk="error",tC=document.createElement("template"),tS=`
  :host([hidden]),
  [hidden],
  ::slotted([hidden]) {
    display: none !important;
...
`;class tI extends HTMLElement{constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tM.content.cloneNode(!0))}get value(){return this.getAttribute("value")}set value(t){this.setAttribute("value",t)}static get observedAttributes(){return["value"]}attributeChangedCallback(t,e,o){"value"===t&&e!==o&&this.#I(this.value)}connectedCallback(){if(this.#R("value"),this.#D(),!tR()){let t=this.shadowRoot.querySelector("web-share");t&&(t.hidden=!0)}}async #I(t){let e,o=this.shadowRoot.querySelector(".result"),i=o?.querySelector(".result__content"),r=o?.querySelector(".result__datetime"),s=o?.querySelector(".result__item");s&&s?.remove();try{let[,o]=await tr();new URL(t),(e=document.createElement("a")).href=t,o?.openWebPageSameTab||(e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")),o?.openWebPage?e.click():window.requestAnimationFrame(()=>e.focus())}catch{e=document.createElement("span")}e.className="result__item",e.part="result__item",e.textContent=t,r.textContent=tO.format(new Date),i?.insertBefore(e,r);let a=o?.querySelector("custom-clipboard-copy"),n=o?.querySelector("web-share");if(a){let e=a.shadowRoot?.querySelector("button");a.setAttribute("value",t),e?.setAttribute("aria-label",`Copy to clipboard ${t}`),a.hidden=!1}if(n&&tR()){let e=n.querySelector("button");n.setAttribute("share-text",t),n.hidden=!1,e?.setAttribute("aria-label",`Share ${t}`)}}#D(){if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;let t=this.shadowRoot.querySelector(".result");t?.animate([{backgroundColor:"var(--highlight)"},{backgroundColor:"transparent"}],{duration:400,easing:"ease-out"})}#R(t){if(Object.prototype.hasOwnProperty.call(this,t)){let e=this[t];delete this[t],this[t]=e}}static defineCustomElement(t="bs-result"){"u">typeof window&&!window.customElements.get(t)&&window.customElements.define(t,tI)}}tI.defineCustomElement();class tD extends HTMLElement{#q=null;#F=null;#B=[];#P;constructor(){super()}get supportedFormats(){return this.#B}set supportedFormats(t){this.#B=t,this.#H()}async connectedCallback(){this.#R("supportedFormats"),this.#q=this.querySelector("#formatsList"),this.#F=this.querySelector("form");let[,t]=await tr();this.#P=t??{},this.#F?.querySelectorAll('[name="general-settings"]').forEach(t=>{t.checked=this.#P[t.value]})}#H(){if(!this.#q)return;let t=this.#P?.formats;this.#q.replaceChildren(),this.supportedFormats.forEach(e=>{let o=document.createElement("li"),i=document.createElement("label"),r=document.createElement("input");r.type="checkbox",r.name="formats-settings",r.value=e,r.checked=null==t||t.includes(e),i.appendChild(r),i.appendChild(document.createTextNode(e)),o.appendChild(i),this.#q.appendChild(o)})}#R(t){if(Object.prototype.hasOwnProperty.call(this,t)){let e=this[t];delete this[t],this[t]=e}}static defineCustomElement(t="bs-settings"){"u">typeof window&&!window.customElements.get(t)&&window.customElements.define(t,tD)}}tD.defineCustomElement();let tq=`
  :host {
...
`;class tB extends HTMLElement{#V=null;#$=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tF.content.cloneNode(!0))}async connectedCallback(){this.#V=this.shadowRoot?.getElementById("historyList"),this.#$=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#j((await ta())[1]||[]),this.#V?.addEventListener("click",this.#U),this.#$?.addEventListener("click",this.#G)}disconnectedCallback(){this.#V?.removeEventListener("click",this.#U),this.#$?.removeEventListener("click",this.#G)}async add(t){if(!t)return;let e={type:"add",message:"Error adding barcode to history"},[o,i=[]]=await ta();if(o||!Array.isArray(i))return this.#W("bs-history-error",e),o;if(i.find(e=>e===t))return;let r=[...i,t],[s]=await tn(r);return s?(td("Error setting history",s),this.#W("bs-history-error",e),s):(this.#V?.insertBefore(this.#Y(t),this.#V.firstElementChild),this.#W("bs-history-success",{type:"add",message:"Barcode added to history"}),null)}async remove(t){if(!t)return;let e={type:"remove",message:"Error removing barcode from history"},[o,i=[]]=await ta();if(o||!Array.isArray(i))return this.#W("bs-history-error",e),o;let r=i.filter(e=>e!==t),[s]=await tn(r);if(s)return td("Error setting history",s),this.#W("bs-history-error",e),s;let a=this.#V?.querySelector(`li[data-value="${t}"]`);return a?.remove(),this.#W("bs-history-success",{type:"remove",message:"Barcode removed from history"}),null}async empty(){let[t]=await tn([]);return t?(td("Error setting history",t),this.#W("bs-history-error",{type:"empty",message:"Error emptying history"}),t):(this.#V?.replaceChildren(),this.#W("bs-history-success",{type:"empty",message:"History emptied successfully"}),null)}#j(t){if(!this.#V)return;this.#V.replaceChildren();let e=document.createDocumentFragment();[...t].reverse().forEach(t=>e.appendChild(this.#Y(t))),this.#V.appendChild(e)}#Y(t){let e,o=document.createElement("li");o.setAttribute("data-value",t);try{new URL(t),(e=document.createElement("a")).href=t,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=t;let i=document.createElement("div");i.className="actions";let r=document.createElement("custom-clipboard-copy"),s=r.shadowRoot?.querySelector("button");r.setAttribute("only-icon",""),r.setAttribute("value",t),s?.setAttribute("aria-label",`Copy to clipboard ${t}`),i.appendChild(r);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("aria-label",`Remove from history ${t}`),a.innerHTML=`
    ...
    `,i.appendChild(a),o.appendChild(e),o.appendChild(i),o}#U=async t=>{let e=t.target;if(e.closest('[data-action="delete"]')){let t=e.closest("li").dataset.value;window.confirm(`Delete history item ${t}?`)&&this.remove(t)}};#G=async()=>{window.confirm("Empty history? This action cannot be undone.")&&this.empty()};#W(t,e=null){let o=new CustomEvent(t,{bubbles:!0,composed:!0,detail:e});this.dispatchEvent(o)}static defineCustomElement(t="bs-history"){"u">typeof window&&!window.customElements.get(t)&&window.customElements.define(t,tB)}}tB.defineCustomElement(),async function(){let t=document.querySelector("a-tab-group"),e=document.querySelector("video-capture"),o=document.querySelector("bs-settings"),i=document.querySelector("bs-history"),r=document.getElementById("cameraPanel").querySelector(".results"),s=document.getElementById("filePanel").querySelector(".results"),a=document.getElementById("scanInstructions"),n=document.getElementById("scanBtn"),l=document.getElementById("dropzone"),d=document.querySelector("resize-observer"),c=document.getElementById("scanFrame"),h=document.getElementById("torchButton"),u=document.getElementById("globalActions"),m=document.getElementById("historyBtn"),p=document.getElementById("historyDialog"),b=document.getElementById("settingsBtn"),g=document.getElementById("settingsDialog"),v=document.getElementById("settingsForm"),f=document.getElementById("cameraSelect"),y=document.getElementById("playVideoButton"),w=null,E=!0;"function"==typeof HTMLDialogElement&&(u?.removeAttribute("hidden"),p?.removeAttribute("hidden"),g?.removeAttribute("hidden"));let{barcodeReaderError:A}=await tv.setup();if(A){O(),u?.setAttribute("hidden",""),t?.setAttribute("hidden",""),ty(`
    ...
    `,{variant:"danger",announce:"alert",trustDangerousInnerHTML:!0,duration:1/0});return}let _=await tv.getSupportedFormats(),[,x]=await tr(),L=x?.formats||_,k=await tv.create(L);e.addEventListener("video-capture:video-play",M,{once:!0}),e.addEventListener("video-capture:error",function(t){let{source:e,reason:o,error:i}=t.detail;if("playback"===e&&"user-gesture-required"===o)return void y?.removeAttribute("hidden");let r="<strong>Unable to start camera</strong><br>An unexpected error occurred while starting the video stream.";"NotFoundError"===i.name?r="<strong>No camera found</strong><br>No compatible camera is available.":"camera"===e&&"camera-access-denied"===o&&(r="<strong>Error accessing camera</strong><br>Permission to use the camera was denied. Please enable camera access in your browser settings."),ty(r,{duration:1/0,variant:"danger",announce:"alert",trustDangerousInnerHTML:!0})},{once:!0}),t_.defineCustomElement();let C=e?.shadowRoot,S=C?.querySelector("video"),T=C?.querySelector('[part="actions-container"]');async function z(){if(E){tl("Scanning..."),a?.removeAttribute("hidden");try{let[,t]=await tr(),e=await k.detect(S),o=e?.rawValue??"";if(!o)throw Error("No barcode detected");if(tc(r,o),t?.addToHistory&&i?.add(o),tp(),!t?.continueScanning){O(),n?.removeAttribute("hidden"),c?.setAttribute("hidden",""),T?.setAttribute("hidden","");return}}catch{}E&&(w=setTimeout(()=>{w=null,z()},1e3))}}function R(){E=!0,null===w&&z()}function O(){E=!1,null!==w&&(clearTimeout(w),w=null)}async function N(t){if(!t)return;let[,e]=await tr(),o=new Image,r=new FileReader;r.onload=r=>{let a=r.target.result;o.onload=async()=>{try{let t=await k.detect(o),r=t?.rawValue??"";if(!r)throw Error("No barcode detected");tc(s,r),e?.addToHistory&&i?.add(r),tp()}catch(t){td(t),ty("<strong>No barcode detected</strong><br><small>Please try again with a different image.</small>",{variant:"danger",announce:"alert",trustDangerousInnerHTML:!0}),tp({success:!1})}},o.src=a,o.alt="Image preview",l.replaceChildren();let n=document.createElement("div");n.className="dropzone-preview";let d=document.createElement("div");d.className="dropzone-preview__image-wrapper";let c=document.createElement("div");c.className="dropzone-preview__file-name",c.textContent=t.name,d.appendChild(o),n.appendChild(d),n.appendChild(c),l.prepend(n)},r.readAsDataURL(t)}async function M(t){y?.setAttribute("hidden",""),c?.removeAttribute("hidden"),tb(t.detail.video,c),R();let o=t.target.getTrackSettings(),i=t.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(h?.addEventListener("click",D),h?.removeAttribute("hidden"),e.hasAttribute("torch")&&tf({el:h,isTorchOn:!0})),o?.zoom&&i?.zoom){let t=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=o?.zoom||1;t?.addEventListener("click",t=>{let o=t.target.closest('[data-action="zoom-in"]'),i=t.target.closest('[data-action="zoom-out"]');o&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n.toFixed(1),e.zoom=n}),t?.removeAttribute("hidden"),r.textContent=n.toFixed(1)}let s=await t_.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,f.appendChild(o)}),s.length>1&&(f?.addEventListener("change",q),f?.removeAttribute("hidden"))}async function I(t){t.preventDefault();let e={},o=new FormData(v),i=o.getAll("general-settings"),r=o.getAll("formats-settings");i.forEach(t=>e[t]=!0),e.formats=r,ts(e),"formats-settings"===t.target.name&&(k=await tv.create(r))}function D(t){e.torch=!e.torch,tf({el:t.currentTarget,isTorchOn:e.hasAttribute("torch")})}function q(t){if("function"!=typeof e.restartVideoStream)return;let o=t.target.value||void 0;e.restartVideoStream(o)}l.accept=X.join(","),o.supportedFormats=_,n.addEventListener("click",function(){n?.setAttribute("hidden",""),c?.removeAttribute("hidden"),T?.removeAttribute("hidden"),R()}),t.addEventListener("a-tab-show",function(t){let e=t.detail.tabId,o=document.querySelector("video-capture");if("cameraTab"===e){if(!o)return;!o.loading&&n.hasAttribute("hidden")&&(c?.removeAttribute("hidden"),T?.removeAttribute("hidden"),R());let t=f?.value||void 0;o.startVideoStream?.(t)}else if("fileTab"===e){if(O(),!o)return;o.stopVideoStream?.(),c?.setAttribute("hidden",""),T?.setAttribute("hidden","")}}),l.addEventListener("files-dropzone-drop",function(t){N(t.detail.acceptedFiles[0])}),d.addEventListener("resize-observer:resize",function(){tb(e.shadowRoot.querySelector("video"),c)}),b.addEventListener("click",function(){g.open=!0}),v.addEventListener("change",function(t,e=0,o=!1){let i=null;if("function"!=typeof t)throw TypeError("Expected a function for first argument");return(...r)=>{clearTimeout(i),o&&!i&&t(...r),i=setTimeout(()=>{i=null,o||t(...r)},e)}}(I,500)),m.addEventListener("click",function(){p.open=!0}),y.addEventListener("click",function(){e.loading||(e?.playVideo?.({emit:!0}),y?.setAttribute("hidden",""))}),document.addEventListener("visibilitychange",function(){if("cameraTab"===t.querySelector("[selected]").getAttribute("id"))if("hidden"===document.visibilityState)O(),e?.stopVideoStream?.();else{let t=document.querySelector("video-capture");if(!t)return;!t.loading&&n.hasAttribute("hidden")&&R();let e=f?.value||void 0;t.startVideoStream?.(e)}}),document.addEventListener("keydown",function(e){let o,i,r,s;"Escape"===e.key&&(o=t.querySelector("#cameraTab").hasAttribute("selected"),i=!n.hidden,r=g.hasAttribute("open"),s=p.hasAttribute("open"),i&&o&&!(r||s)&&n.click())}),document.addEventListener("bs-history-success",function(t){let{type:e,message:o}=t.detail;"add"===e&&ty(o,{variant:"success"})}),document.addEventListener("bs-history-error",function(t){let{type:e,message:o}=t.detail;("remove"===e||"empty"===e)&&p?.hide(),ty(o,{variant:"danger",announce:"alert"})})}();
//# sourceMappingURL=barcode-scanner.40726811.js.map
